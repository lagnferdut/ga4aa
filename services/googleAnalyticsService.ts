...
export async function listAccounts(token: string): Promise<GaAccount[]> {
    const data = await fetchWithAuth(`${ADMIN_API_URL}/accountSummaries`, token);
    return data.accountSummaries.map((summary: any) => ({
        name: summary.account,
        displayName: summary.displayName,
    }));
}

export async function listProperties(accountName: string, token: string): Promise<GaProperty[]> {
    const data = await fetchWithAuth(`${ADMIN_API_URL}/accountSummaries?parent=${accountName}`, token);
    const accountSummary = data.accountSummaries.find((summary: any) => summary.account === accountName);
    if (!accountSummary || !accountSummary.propertySummaries) return [];
    
    return accountSummary.propertySummaries.map((summary: any) => ({
...
const getReportRequestConfig = (analysisType: AnalysisType): object => {
...
export async function runReport(propertyId: string, token: string, analysisType: AnalysisType): Promise<string> {
    ...
    const headers = [...data.dimensionHeaders.map((h: any) => h.name), ...data.metricHeaders.map((h: any) => h.name)];
    let csvString = headers.join(',') + '\n';

    data.rows.forEach((row: any) => {
        const dimensionValues = row.dimensionValues.map((dv: any) => dv.value).join(',');
        const metricValues = row.metricValues.map((mv: any) => mv.value).join(',');
