...
import { analyzeGaData } from './services/geminiService.js';
import { listAccounts, listProperties, runReport } from './services/googleAnalyticsService.js';
import { AnalysisType } from './types.js';
import { ANALYSIS_OPTIONS } from './constants.js';
import { SparklesIcon, ClipboardIcon, ChartBarIcon, DocumentTextIcon, LightBulbIcon, CheckIcon, UsersIcon, ArrowPathIcon } from './components/icons.js';
...
const GoogleLoginButton = ({ onLoginSuccess, clientId, disabled }) => {
    const handleGoogleLogin = useCallback(() => {
        ...
        if (!clientId) {
            alert("Identyfikator klienta Google nie został załadowany. Spróbuj odświeżyć stronę.");
            return;
        }
    ...
    return (
        <button
            ...
            disabled={disabled}
            className="w-full flex justify-center items-center gap-3 text-lg font-bold bg-white text-gray-800 py-3 px-6 rounded-lg hover:bg-gray-200 transition-colors duration-200 disabled:opacity-50 disabled:cursor-wait"
        >
            ...
            {disabled ? 'Ładowanie konfiguracji...' : 'Zaloguj się z Google'}
        </button>
    );
};

const AnalyticsSelector = ({ accounts, properties, selectedAccount, onAccountChange, selectedProperty, onPropertyChange, disabled }) => (
...
const AnalysisOptions = ({ selectedType, setSelectedType, disabled }) => (
...
export default function App() {
  ...
  const [gaState, setGaState] = useState({ accounts: INITIAL_STATE.accounts, properties: INITIAL_STATE.properties, selectedAccount: INITIAL_STATE.selectedAccount, selectedProperty: INITIAL_STATE.selectedProperty });
  ...
  const [config, setConfig] = useState({ googleClientId: null, loading: true });

  useEffect(() => {
    fetch('/api/config')
        .then(res => {
            if (!res.ok) throw new Error('Nie udało się pobrać konfiguracji serwera.');
            return res.json();
        })
        .then(data => setConfig({ googleClientId: data.googleClientId, loading: false }))
        .catch(err => {
            console.error("Failed to fetch config", err);
            setAnalysisState(s => ({...s, error: 'Nie udało się załadować konfiguracji aplikacji. Sprawdź, czy zmienne środowiskowe na serwerze są poprawnie ustawione.'}));
            setConfig({ googleClientId: null, loading: false });
        });
  }, []);
          ...
          <div className="w-full p-6 md:p-8 bg-black/20 border border-brand-secondary rounded-xl shadow-2xl flex flex-col gap-8">
              {!authState.isAuthenticated ? (
                <GoogleLoginButton 
                  ...
                  disabled={config.loading || !config.googleClientId}
                />
              ) : (
                <>
                    <AnalyticsSelector 
